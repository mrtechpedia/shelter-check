...
const DecorBox = ({ CommonDecorProps }) => {
  ...
  const [outOfFour, setOutOfFour] = useState(
    // CommonDecorProps.outOfFourr[0].des
  );
  const [price, setPrice] = useState(
    CommonDecorProps.outOfFourr[0].desPrice
  );

  function getPrice(event) {
    CommonDecorProps.outOfFourr.forEach((object) => {
  ...
  return (
    <div className="DecorBox">
      <form>
        ...
        <h1 className="headinggg">{CommonDecorProps.name}</h1>
        <div className="content">
          ...
          <label for="quantity">{`${CommonDecorProps.name}s Quantity`}</label>
          <br></br>
          <input
            type="text"
            className="input"
          ...
          <select
            ...
            onClick={getPrice}
          >
            <option>{CommonDecorProps.outOfFourr[0].des}</option>
            <option>{CommonDecorProps.outOfFourr[1].des}</option>
            <option>{CommonDecorProps.outOfFourr[2].des}</option>
            <option>{CommonDecorProps.outOfFourr[3].des}</option>
          </select>

          <br></br>
          <br></br>
        </div>
        <Button variant="primary" className="buttond" onClick={DoIt}>
          Book Now
        </Button>{" "}
      </form>
    </div>
